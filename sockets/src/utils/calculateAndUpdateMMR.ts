...
export async function calculateAndUpdateMMR(
    ...
    const winnerGain = !draw ? calculateGain(winnerMMR, loserMMR) : calculateDraw(winnerMMR, loserMMR);
    const loserGain = !draw ? -calculateGain(loserMMR, winnerMMR) : calculateDraw(loserMMR, winnerMMR);
