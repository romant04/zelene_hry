...
const CARDS = [
    "c_sedma", "c_osma", "c_devitka", "c_desitka", "c_eso", "c_kral", "c_spodek", "c_svrsek",
    "e_sedma", "e_osma", "e_devitka", "e_desitka", "e_eso", "e_kral", "e_spodek", "e_svrsek",
    "z_sedma", "z_osma", "z_devitka", "z_desitka", "z_eso", "z_kral", "z_spodek", "z_svrsek",
    "k_sedma", "k_osma", "k_devitka", "k_desitka", "k_eso", "k_kral", "k_spodek", "k_svrsek",
]
...
function prepareCardDeck() {
    const deck: Card[] = shuffle([...CARDS].map((card) => {
...
export function setupPrsiNamespace(io: Server) {
    ...
    prsiNamespace.on("connection", (socket: Socket) => {
        ...
        const id = generalGameData?.players.find(player => player.token === token)!.id;
        ...
        socket.on("drawCard", (card: Card) => {
            ...
            gameState.deck = gameState.deck.filter(c => !(c.suit === card.suit && c.rank === card.rank));
            gameState.currentPlayerId = enemy!.id; // Switch to the next player
            ...
            if (gameState.deck.length === 0) {
                ...
                gameState.centerCards = [gameState.centerCards![gameState.centerCards!.length - 1]];
        ...
        socket.on("changeSuit", (data: {suit: string, old: string}) => {
            ...
            gameState.centerCards?.push({ suit: data.old as "c" | "e" | "z" | "k", rank: "svrsek" });
            player!.cards = player!.cards.filter(c => !(c.suit === data.old && c.rank === "svrsek"));
        ...
        socket.on("disconnect", () => {
            ...
            if (gameData) {
                ...
                if (gameData.players?.filter(player => player.isConnected).length === 0) {
                    // If all players are disconnected, remove the game data
                    PrsiGameData.delete(gameId);
                    GameRoomsMap.delete(gameId);
                    console.log(`All players disconnected, removing game data for gameId: ${gameId}`);
                }
            }
        })
    });
